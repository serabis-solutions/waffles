...
fn run() -> Result<()> {
    ...
    config
        .merge(config::File::with_name("Waffles")).expect("Unable to load Waffles.toml")
        .merge(config::Environment::with_prefix("WAFFLES")).expect("Failed to read ENV");

    let addr = format!("{}:{}",
        config.get::<String>("listen.address").expect("listen::address missing from config?"),
        config.get::<String>("listen.port").expect("listen::port missing from config?"));
    ...
    let server = connections.for_each(|(sock, remote_addr)| {
        let service = proxy::Proxy{ config: config.clone(), client: client.clone() };
