...
fn proxy_request(config: &Config, client: &Client<HttpConnector, Body>, req: Request) -> Box<Future<Item=Response, Error=hyper::Error>> {
    let proxy_url = format!(
        "http://{}:{}",
        config.get::<String>("proxy.address").expect("unable to load proxy address from config"),
        config.get::<String>("proxy.port").expect("unable to load proxy address from config")
    );

    debug!("Proxy url: {}", proxy_url);
    let mut proxied_request :Request = hyper::client::Request::new(
        req.method().clone(), 
        proxy_url.parse::<hyper::Uri>().expect("Unable to parse URI")
    );
